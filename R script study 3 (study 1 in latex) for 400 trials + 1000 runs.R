# R script study 3 (study 1 in latex) for 400 trials + 1000 runs to find choice probabilities at same stage as behavioural experiments
...
# for multiple runs: 
for(rep in 1:2){ # 1000 replications
  
  print(paste0("Replication No.", rep))
  set.seed(rep)
# train model:
  # task 1:
s3traintask1 <- createTrainingData(s3dattask1, nruns = 1, random = FALSE)
wm_task1s3 <- RWlearning(s3traintask1)
# task 2:
s3traintask2 <- createTrainingData(s3dattask1, nruns = 1, random = TRUE)
wm_task2s3 <- RWlearning (s3traintask2, wm = wm_task1s3)
# block 1:
s3trainblock1 <- createTrainingData(s3datblock1, nruns = 1, random = TRUE)
wm_block1s3 <- RWlearning(s3trainblock1, wm = wm_task2s3)
# block 2:
s3trainblock2 <- createTrainingData(s3datblock1, nruns = 1, random = TRUE)
wm_block2s3 <- RWlearning(s3trainblock2, wm = wm_block1s3)
# block 3:
s3trainblock3 <- createTrainingData(s3datblock1, nruns = 1, random = TRUE)
wm_block3s3 <- RWlearning(s3trainblock3, wm = wm_block2s3)
# block 4:
s3trainblock4 <- createTrainingData(s3datblock1, nruns = 1, random = TRUE)
...
# saving the sd
sd_cueset1_acc_400trials <- sd(resultstudy3.400trials.multipleruns$cueset1_acc_400trials, na.rm = TRUE)
sd_cueset1_prep_400trials <- sd(resultstudy3.400trials.multipleruns$cueset1_prep_400trials, na.rm = TRUE)
sd_HFcueset2_acc_400trials <- sd(resultstudy3.400trials.multipleruns$HFcueset2_acc_400trials, na.rm = TRUE)
sd_LFcueset2_acc_400trials <- sd(resultstudy3.400trials.multipleruns$LFcueset2_acc_400trials, na.rm = TRUE)
sd_HFcueset2_prep_400trials <- sd(resultstudy3.400trials.multipleruns$HFcueset2_prep_400trials, na.rm = TRUE)
sd_LFcueset2_prep_400trials <- sd(resultstudy3.400trials.multipleruns$LFcueset2_prep_400trials, na.rm = TRUE)
