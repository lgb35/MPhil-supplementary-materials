...
library(magrittr)
# read csv:
datstudy1 <- read_csv("csv for 70:30 ratio det.csv") # the code for study 2.
...
datstudy1
trainforstudy1_4000trials <- createTrainingData(datstudy1, nruns = 200, random=TRUE) # trained for 4000 trials i.e. until asymptote
...
lines(constantPrep4000[,1], col="red", lty=3) 
abline(h=0)
abline(v=400)
axis(1, at = seq(0,4000, by =500))
legend(2000, -0.1, legend = c("cue set 1 + HF cue set 2", "cue set 1 + LF cue set 2", "constant cue"),
