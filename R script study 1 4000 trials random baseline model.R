# r script study 1 (study 2 in latex) for random baseline model
...
datstudy1
datstudy1_shuffled <- do.call(rbind, replicate(200, datstudy1, simplify = FALSE))
datstudy1_shuffled
# start loop
for(rep in 1:1000){ # 1000 replications reported in paper
  ...
  set.seed(rep)
# shuffle the data
  datstudy1_shuffled$Outcomes <- sample(datstudy1_shuffled$Outcomes)
  datstudy1_shuffled$Cues <- sample(datstudy1_shuffled$Cues)
  datstudy1_shuffled
  # train model
  trainforstudy1_4000trials_shuffled <- createTrainingData(datstudy1_shuffled, random=TRUE)
...
# for cue set 1 + HF cue set 2, Acc vs. Prep:
