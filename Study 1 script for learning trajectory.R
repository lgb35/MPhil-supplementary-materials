# Study 1 script for learning trajectory.
...
datforst3 <- read_csv("csv for study 1 4000 trials.csv") # has 20 trials in it.
...
# plot:
par(mfrow=c(1,2))
plot(AWs3a[,1], col="#F75C03", type = "l", ylim =c(-.5, 1), xlim = c(0, 4000), xaxt = "n", xlab = "Training trials", ylab="Learned weight")
#mtext("a) compound cues to 'Acc'", side = 3, adj=0, line=1.2, cex = 1.2, font=2)
lines(BYs3a[,1], col="#820263", lty=2)
lines(AXs3a[,1], col="#FFB2E6", lty = 1)
lines(BZs3a[,1], col="#04A777", lty = 2)
...
legend(2000, -0.05, legend = c("cue set 1 + HF cue set 2" , "cue set 1 + LF cue set 2", "constant cue"),
       col=c("#F75C03", "#FFB2E6", "red"), lty=1, bg="white", cex = 1, xjust = 0.5)
...
# plot:
plot(BYs3b[,1], col="#820263", type = "l", ylim =c(-.5, 1), xlim = c(0, 4000), xaxt = "n", xlab = "Training trials", ylab="Learned weight")
#mtext("a) compound cues to 'Prep'", side = 3, adj=0, line=1.2, cex = 1.2, font=2)
lines(AWs3b[,1], col="#F75C03", lty=2)
lines(AXs3b[,1], col="#FFB2E6", lty = 2)
lines(BZs3b[,1], col="#04A777", lty = 1)
lines(constantPrep40003b[,1], col="red", lty=3) # ignore if don't want to add constant
abline(h=0)
abline(v=400)
axis(1, at = seq(0,4000, by =500))
legend(2000, -0.05, legend = c("cue set 1 + HF cue set 2" , "cue set 1 + LF cue set 2", "constant cue"),
       col=c("#820263", "#04A777", "red"), lty=1, bg="white", cex = 1, xjust = 0.5)
...
labels2 <- c("Cue set 1\n",
             ...
             "LF cue set 2\n")
# add labels: 
text(x =colMeans(bp2), y = -0.1, labels = labels2, 
     #srt = 45,
