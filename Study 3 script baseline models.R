# Study 3 script baseline models:
...
# read csv:
datstudy2 <- read_csv("csv for 70:30 ratio prob.csv") # has 200 trials.
...
datstudy2
datstudy2_shuffled <- do.call(rbind, replicate(20, datstudy2, simplify = FALSE)) # 4000 trials
datstudy2_shuffled
# start loop
...
# make the training data
  ...
  set.seed(rep)
  # shuffle the data
  datstudy2_shuffled$Outcomes <- sample(datstudy2_shuffled$Outcomes) # shuffle
  datstudy2_shuffled$Cues <- sample(datstudy2_shuffled$Cues)
  datstudy2_shuffled
  # train model
  trainforstudy2_4000trials_shuffled <- createTrainingData(datstudy2_shuffled, random=TRUE) # randomise
  wm_prob4000_shuffled <- RWlearning(trainforstudy2_4000trials_shuffled)
# calculate choice probabilities:
# for HF cue set 1 + HF cue set 2 for Acc outcome:
  # AW:
  ...
  options1_shuffled
  # for HF cue set 1 + HF cue set 2 for Prep outcome:
  # BY
  ...
  options2_shuffled
  # for HF cue set 1 + LF cue set 2 for Acc outcome:
  # BW:
  ...
  options3_shuffled
  # for HF cue set 1 + LF cue set 2 for Prep outcome:
  # AY:
  ...
  options4_shuffled
  # for LF cue set 1 + HF cue set 2 for Acc outcome:
  # AX:
  ...
  options5_shuffled
  # for LF cue set 1 + HF cue set 2 for Prep outcome:
  # BZ:
  ...
  options6_shuffled
  # for LF cue set 1 + LF cue set 2 for Acc outcome:
  # BX
  ...
  options7_shuffled
  # for LF cue set 1 + LF cue set 2 for Prep outcome:
  # AZ:
...
obs_LFcueset1LFcueset2prep <- LFcueset1LFcueset2_prep_4000trials_study2
shuffled_prep_LFcueset1LFcueset2 <- resultstudy2.4000trials.baseline$LFcueset1LFcueset2_prep_4000trials_study2_shuffled
# calculate:
p.prep.LFcueset1LFcueset2 <- mean(abs(shuffled_prep_LFcueset1LFcueset2 - mean(shuffled_prep_LFcueset1LFcueset2)) >= abs(obs_LFcueset1LFcueset2prep - mean(shuffled_prep_LFcueset1LFcueset2)))
p.prep.LFcueset1LFcueset2
